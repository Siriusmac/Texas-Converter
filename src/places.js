...
export function extractAreas(entity, language='it') {
  ...
  return claims.filter(c => !preferred || c.rank === 'preferred').flatMap(c => {
    ...
    const dates = [['P585', 'Dato del'], ['P580', 'Dal'], ['P582', 'Fino al']].flatMap(([p, label]) => (qualifiers[p] || []).map(s => dateLabel(s,language)).filter(Boolean).map(d => `${t(label,language)} ${d}`));
    return [{ km2, date: dates.join(' · ') || t('Data non indicata',language), referenced: Boolean(c.references?.length) }];
  });
}

export function extractPlace(entity, language='it') {
  ...
  return {
    id: entity.id,
    name: entity.labels?.[language]?.value || entity.labels?.en?.value || entity.labels?.it?.value || entity.id,
...
export async function searchPlaces(query, {signal, fetcher = fetch, language = 'it'} = {}) {
  const text = query.trim();
  if (text.length < 2 || text.length > 80) throw new Error(t('Inserisci da 2 a 80 caratteri, per esempio “Padova” o “Veneto”.',language));
  const key = language + ':' + text.toLocaleLowerCase(language);
  ...
  async function request(params) {
    ...
    if (!response.ok) throw new Error(t('La ricerca online non è disponibile adesso. Riprova tra poco.',language));
    const data = await response.json();
    if (data.error) throw new Error(t('Wikidata non riesce a completare la ricerca. Riprova tra poco.',language));
    return data;
  }
  try {
    const found = await request({action: 'wbsearchentities', search: text, language, uselang: language, type: 'item', limit: '8'});
    const ids = (found.search || []).map(e => e.id).filter(id => /^Q\d+$/.test(id));
    if (!ids.length) return [];
    const data = await request({action: 'wbgetentities', ids: ids.join('|'), props: 'labels|descriptions|claims', languages: language==='it'?'it|en':'en'});
    const results = ids.map(id => data.entities?.[id]).filter(Boolean).map(entity=>extractPlace(entity,language)).filter(Boolean);
    if (cache.size >= 20) cache.delete(cache.keys().next().value);
    cache.set(key, {results, expires: Date.now() + 300000});
    return results;
  } catch (error) {
    if (controller.signal.aborted && !signal?.aborted) throw new Error(t('La ricerca sta impiegando troppo tempo. Riprova.',language));
    if (signal?.aborted) throw error;
    if (error instanceof TypeError || error instanceof SyntaxError) throw new Error(t('Impossibile leggere la risposta di Wikidata. Controlla la connessione e riprova.',language));
    throw error;
  } finally {
    clearTimeout(timer);
