...
test('imperial submissions use the same calculation as the converter',async()=>{
 const {convert}=await import('../src/converter.js');
 for(const mode of ['length','area'])for(const unit of ['in','ft','yd','mi']){
  const event={id:crypto.randomUUID(),at:Date.now(),mode,unit,value:773};
  assert.equal(measurement(event).amount,String(scaledInteger(convert(event.value,unit,mode))));
 }
});
