import {UNITS,toTexas} from '../src/units.js';
...
export function measurement(body,now=Date.now()){
 if(!body||!['length','area'].includes(body.mode)||!Object.hasOwn(UNITS,body.unit)||typeof body.value!=='number'||!Number.isFinite(body.value)||body.value<=0||typeof body.id!=='string'||! /^[0-9a-f-]{36}$/i.test(body.id)||!Number.isFinite(body.at)||Math.abs(now-body.at)>300000)throw new Error('Invalid measurement');
 const ratio=toTexas(body.value,body.unit,body.mode);
